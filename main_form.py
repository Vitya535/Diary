...
class Calendar(HTMLCalendar):
    ...
    def add_record():
        ...
        add_time = split[2] + '-' + str(utils.tuple_for_months_gen_case.index(split[1]) + 1) + '-' + split[0] + \
                   ' 0' + split[3] + ':00.000'
        con = connect("data_for_diary.db")
        cursor = con.cursor()
    ...
    def inst():
        ...
        if Calendar._instance is None:
            ...
            Calendar.first_type_day_in_month = utils.get_first_type_day_in_month(Calendar.inst(), Calendar.Year,
                                                                                 Calendar.Month, Calendar.Day)
            Calendar.last_type_day_in_month = utils.get_last_type_day_in_month(Calendar.inst(), Calendar.Year,
                                                                               Calendar.Month, Calendar.Day)
